...
try {
    ...
    foreach ($phase in @('seed','verify','voice')) {
        $phaseData = if ($phase -eq 'voice') { Join-Path $data 'voice' } else { $data }
    ...
    $report = @{ passed = $true; singleExecutable = $true; processExited = $true; phases = @('seed','verify','voice'); evidenceDirectory = (Split-Path -Leaf $data) }
