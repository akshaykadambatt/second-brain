param([string]$Executable = '',
    [ValidateSet('seed','verify','voice','flow','timed','study','replay','audio','transcription','hybrid','assistant','knowledge','history','companion','stream')]
    [string[]]$Phases = @('seed','verify','voice','flow','timed','study','replay','audio','transcription','hybrid','assistant','knowledge','history','companion','stream'))
...
try {
    ...
    foreach ($phase in $Phases) {
        $phaseData = if ($phase -in @('voice','flow','timed','study','replay','stream','audio','transcription','hybrid','assistant','knowledge','history','companion')) { Join-Path $data $phase } else { $data }
        $p = Start-Process -FilePath $isolatedExe -ArgumentList @('--data-dir', ('"' + $phaseData + '"'), '--smoke-test', $phase) -PassThru -WindowStyle Hidden
        if (-not $p.WaitForExit($(if ($phase -eq 'history') { 90000 } else { 30000 }))) { $p.Kill(); throw "Smoke phase timed out: $phase" }
