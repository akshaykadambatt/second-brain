param([string]$Executable = '',
    [ValidateSet('seed','verify','voice','flow','timed','study','replay','audio','transcription','hybrid','assistant','knowledge','history','storage','wrap','companion','stream','tray','shell')]
    [string[]]$Phases = @('seed','verify','voice','flow','timed','study','replay','audio','transcription','hybrid','assistant','knowledge','history','storage','wrap','companion','stream','tray','shell'))
...
try {
    ...
    foreach ($phase in $Phases) {
        $phaseData = if ($phase -in @('voice','flow','timed','study','replay','stream','audio','transcription','hybrid','assistant','knowledge','history','storage','wrap','companion','tray','shell')) { Join-Path $data $phase } else { $data }
