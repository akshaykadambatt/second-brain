param([string]$Executable = '',
    [ValidateSet('seed','verify','voice','flow','timed','study','replay','audio','transcription','hybrid','assistant','knowledge','history','storage','wrap','companion','stream','tray','shell','chrome','shortcuts','context','latency','opening','flowing','rich','speakers','capture','names','meet','imports','pdf','word','powerpoint','clientknowledge','requests','memory','scoped')]
    [string[]]$Phases = @('seed','verify','voice','flow','timed','study','replay','audio','transcription','hybrid','assistant','knowledge','history','storage','wrap','companion','stream','tray','shell','chrome','shortcuts','context','latency','opening','flowing','rich','speakers','capture','names','meet','imports','pdf','word','powerpoint','clientknowledge','requests','memory','scoped'))
...
try {
    ...
    foreach ($phase in $Phases) {
        $phaseData = if ($phase -in @('voice','flow','timed','study','replay','stream','audio','transcription','hybrid','assistant','knowledge','history','storage','wrap','companion','tray','shell','chrome','shortcuts','context','latency','opening','flowing','rich','speakers','capture','names','meet','imports','pdf','word','powerpoint','clientknowledge','requests','memory','scoped')) { Join-Path $data $phase } else { $data }
        $p = Start-Process -FilePath $isolatedExe -ArgumentList @('--data-dir', ('"' + $phaseData + '"'), '--smoke-test', $phase) -PassThru -WindowStyle Hidden
        if (-not $p.WaitForExit($(if ($phase -in @('history','storage','pdf')) { 90000 } else { 30000 }))) { $p.Kill(); throw "Smoke phase timed out: $phase" }
