param([string]$Executable = '',
    [ValidateSet('seed','verify','voice','flow','timed','study','replay','audio','transcription','stream')]
    [string[]]$Phases = @('seed','verify','voice','flow','timed','study','replay','audio','transcription','stream'))
...
try {
    ...
    foreach ($phase in $Phases) {
        $phaseData = if ($phase -in @('voice','flow','timed','study','replay','stream','audio','transcription')) { Join-Path $data $phase } else { $data }
