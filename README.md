...
Client record names and aliases retain Unicode during indexing, including non-Latin text.

Long meetings retain a selective, source-linked extractive memory after older turns leave the recent transcript buffer. Decisions, commitments, risks and related wording receive priority; no observation is promoted automatically. **Earlier meeting context** on Live shows retained excerpts. Memory is bounded to 40 excerpts / about 8,000 characters, with a 3,500-character opening-context allowance and a bounded recent window. Older information can still be omitted when these limits fill. Stopping the companion writes a rebuildable, versioned `meeting-memory.json` next to the immutable transcript, with a source hash; backups include it.

Automatic assistance recognizes directed English requests as well as questions, including “Explain the rollout,” “Walk us through the risks,” and “I’d like your recommendation.” It waits for a finalized computer-audio utterance, keeps microphone echo and duplicate suppression, and cancels pending generation at session boundaries. Ordinary plans, reported speech and explicitly negated requests remain excluded by the conservative phrase rules; use the manual question box for missed or ambiguous requests.

**Quiet suggestions** on Live is optional and off by default. It shows one source reminder or follow-up prompt from existing retrieval, at least 30 seconds apart. Cards expire after 30 seconds or a topic change; dismissal and duplicate suppression keep them quiet. Suggestions use a separate card, never select a reader answer, and make no additional model calls. Source reminders remain observations to check, not newly confirmed facts.

**Shorter**, **Explain** and **Example** create separate answer variants from the selected answer's saved context and evidence. **Original answer** follows the exact parent link. A late variant cannot take over a newer question or manual selection, and the primary answer keeps its near-end continuation behavior. Refinements also work after listening stops without restarting capture. **Save client talk track** opens a reviewable Markdown draft for the meeting's client; saving requires a dated, matching source quote. Talk tracks remain observations until explicitly confirmed and can be retrieved by client and aliases.
