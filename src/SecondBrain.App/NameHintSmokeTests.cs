...
internal static class NameHintSmokeTests
{
    ...
    internal static async Task Run(MainWindow main, string directory, Action<bool, string> check)
    {
        ...
        try
        {
            ...
            check(!observed.Supported, "Native accessibility inspection rejects a synthetic window without a Teams document URL");
            ...
            names.Read = (_, _) => new(true, ["Morgan"], "Synthetic single speaker");
            for (var i = 0; i < 6; i++)
            {
            ...
            check(names.Resolve(detail)?.Name == "Morgan", "Fresh stable window observations correlate with a single timed audio speaker");
        ...
        main.Transcriber.ResolveNameHint = detail => detail.Source == AudioSource.System && detail.Words.Length > 0 && detail.Words.All(w => w.SpeakerId is not null)
            ...
            ? new(1, detail.SessionId, detail.SegmentId, SpeakerNameHints.Binding(detail), "Remote fixture", detail.Start, detail.End) : null;
