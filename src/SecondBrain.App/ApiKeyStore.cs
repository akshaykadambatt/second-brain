...
internal sealed class ApiKeyStore(string directory)
{
    public string FilePath => Path.Combine(directory, "deepgram-key.protected");
    public bool Exists => File.Exists(FilePath);
    public string Load()
    {
        if (!Exists) throw new InvalidOperationException("Deepgram key is not configured yet. Ask Codex to finish setup.");
        ...
        catch (CryptographicException) { throw new InvalidOperationException("The Deepgram key belongs to a different Windows account or is damaged. Import it again."); }
    }
