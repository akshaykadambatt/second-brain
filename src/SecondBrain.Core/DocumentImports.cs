...
public static class DocumentImports
{
        ...
        Func<byte[], CancellationToken, ImportExtraction>? pdfExtractor = null, Guid clientId = default)
    {
        ...
        try
        {
            ...
            if (extension is not (".md" or ".txt" or ".pdf" or ".docx" or ".pptx")) throw new InvalidDataException("Choose Markdown, text, PDF, DOCX or PPTX.");
            byte[] bytes;
            using (var input = new FileStream(source, FileMode.Open, FileAccess.Read, FileShare.Read))
            {
                if (input.Length > (extension is ".pdf" or ".docx" or ".pptx" ? 20_000_000 : 2_000_000)) throw new InvalidDataException("Text imports are limited to 2 MB; PDF and Office files to 20 MB.");
